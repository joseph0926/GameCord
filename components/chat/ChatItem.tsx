...
const ChatItem = ({
  ...
  const isModerator = currentMember.role === MemberRole.MANAGER;
    ...
    <div className="group relative flex w-full items-center p-4 transition hover:bg-black/5">
      <div className="group flex w-full items-start gap-x-2">
        ...
        <div className="flex w-full flex-col">
          ...
          {!fileUrl && !isEditing && (
            <p
              className={cn('text-sm text-zinc-600 dark:text-zinc-300', deleted && 'mt-1 text-xs italic text-zinc-500 dark:text-zinc-400')}
            >
              {content}
              {isUpdated && !deleted && <span className="mx-2 text-[10px] text-zinc-500 dark:text-zinc-400">(edited)</span>}
            </p>
          )}
          {!fileUrl && isEditing && (
            <Form {...form}>
